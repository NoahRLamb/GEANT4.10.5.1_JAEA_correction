...
//#include "changeJAEA.hh"
#include "newG4JAEAElasticScatteringModel.hh"
...
//Rounding up to half the energy-grid separation (5 keV)
	if (photonEnergy0*1000>=0.5*(Diff_CS_data[Z][0][index]+Diff_CS_data[Z][0][index+1])){if(index<299){index++;}}

///////////////////////////////////////////////////////////////////
//This is where we think the issue is. We replace it with our pdf/cdf below
/*
...
//Here we make a new pdf using our calculations.
	
	G4double corrsum = 0;

	for(int i = 0; i<=180;i++){

		pdf[i]=Diff_CS_data[Z][i+2][index]*CLHEP::twopi*sin(i*deg)*deg/Diff_CS_data[Z][1][index];
		
		corrsum=corrsum+pdf[i];
